	...
	implements Micropolis.Listener
{
	...
	static ResourceBundle cityStrings = ResourceBundle.getBundle("strings/CityStrings");
	static ResourceBundle guiStrings = MainWindow.guiStrings;

	public EvaluationPane(Micropolis _engine)
	{
		super(new BorderLayout());

		JButton dismissBtn = new JButton(guiStrings.getString("dismiss-evaluation"));
	...
	private JComponent makePublicOpinionPane()
	{
		...
		JLabel headerLbl = new JLabel(guiStrings.getString("public-opinion"));
		...
		me.add(new JLabel(guiStrings.getString("public-opinion-1")), c1);

		c1.gridy = 4;
		me.add(new JLabel(guiStrings.getString("public-opinion-2")), c1);
		...
		me.add(new JLabel(guiStrings.getString("public-opinion-yes")), c2);

		c2.gridy = 3;
		me.add(new JLabel(guiStrings.getString("public-opinion-no")), c2);
	...
	private JComponent makeStatisticsPane()
	{
		...
		JLabel headerLbl = new JLabel(guiStrings.getString("statistics-head"));
		Font curFont = headerLbl.getFont();
		headerLbl.setFont(
			curFont.deriveFont(curFont.getStyle() | Font.BOLD, (float)(curFont.getSize() * 1.2))
			);
		...
		JLabel header2Lbl = new JLabel(guiStrings.getString("city-score-head"));
		...
		c2.gridy = c3.gridy = 1;
		me.add(new JLabel(guiStrings.getString("stats-population")), c2);
		popLbl = new JLabel();
		me.add(popLbl, c3);

		c2.gridy = ++c3.gridy;
		me.add(new JLabel(guiStrings.getString("stats-net-migration")), c2);
		deltaLbl = new JLabel();
		me.add(deltaLbl, c3);

		c2.gridy = ++c3.gridy;
		me.add(new JLabel(guiStrings.getString("stats-last-year")), c2);

		c2.gridy = ++c3.gridy;
		me.add(new JLabel(guiStrings.getString("stats-assessed-value")), c2);
		assessLbl = new JLabel();
		me.add(assessLbl, c3);

		c2.gridy = ++c3.gridy;
		me.add(new JLabel(guiStrings.getString("stats-category")), c2);
		cityClassLbl = new JLabel();
		me.add(cityClassLbl, c3);

		c2.gridy = ++c3.gridy;
		me.add(new JLabel(guiStrings.getString("stats-game-level")), c2);
		gameLevelLbl = new JLabel();
		me.add(gameLevelLbl, c3);

		c2.gridy = c3.gridy = 21;
		me.add(new JLabel(guiStrings.getString("city-score-current")), c2);
		scoreLbl = new JLabel();
		me.add(scoreLbl, c3);

		c2.gridy = ++c3.gridy;
		me.add(new JLabel(guiStrings.getString("city-score-change")), c2);
	...
	private void loadEvaluation()
	{
		...
		for (int i = 0; i < voterProblemLbl.length; i++) {
			...
			if (numVotes != 0) {
				voterProblemLbl[i].setText(cityStrings.getString("problem."+p.name()));
	...
	{
		return cityStrings.getString("class."+cityClass);
	}

	static String getGameLevelName(int gameLevel)
	{
		return cityStrings.getString("level."+gameLevel);
	}
}
