...
class TerrainBehavior extends TileBehavior
{
			...
			if (PRNG.nextInt(8) == 0)
			{
				...
				if (isCombustible(c)) {
					if (isZoneCenter(c)) {
						...
						if (c > INDUSTRIAL_ZONE_BASE) { //explode
	...
	static int [] TRAFFIC_DENSITY_TAB = {ROAD_BASE, LTRF_BASE, HTRF_BASE};
	...
	void doRoad()
	{
		...
		if (tden != newLevel)
		{
			int z = ((tile - ROAD_BASE) & 15) + TRAFFIC_DENSITY_TAB[newLevel];
