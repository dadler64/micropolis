...
public class MapGenerator
{
	...
	static final char [] REdTab = new char[] {
		RIVER_EDGE + 8, RIVER_EDGE + 8, RIVER_EDGE + 12, RIVER_EDGE + 10,
		RIVER_EDGE + 0, RIVER,       RIVER_EDGE + 14, RIVER_EDGE + 12,
		RIVER_EDGE + 4, RIVER_EDGE + 6, RIVER,        RIVER_EDGE + 8,
		RIVER_EDGE + 2, RIVER_EDGE + 4, RIVER_EDGE + 0,  RIVER
		};
				...
				if (map[mapY][mapX] == R_EDGE)
				{
					...
					for (int z = 0; z < 4; z++)
					{
						...
						if (engine.testBounds(xtem, ytem) &&
							((map[ytem][xtem] & LOW_MASK) != DIRT) &&
							(((map[ytem][xtem] & LOW_MASK) < WOODS_LOW) ||
							((map[ytem][xtem] & LOW_MASK) > WOODS_HIGH)))
						{
							bitindex |= 1;
						}
					}
	...
	private void treeSplash(int xloc, int yloc)
	{
		...
		for (int z = 0; z < dis; z++)
		{
			...
			if ((map[mapY][mapX] & LOW_MASK) == DIRT)
			{
				map[mapY][mapX] = WOODS;
			}
		}
