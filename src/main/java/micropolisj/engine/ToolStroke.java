...
public class ToolStroke
{
	...
	boolean apply1(ToolEffectIfc eff)
	{
		...
		case RESIDENTIAL:
			return applyZone(eff, RESIDENTIAL_CLEAR);

		case COMMERCIAL:
			return applyZone(eff, COMMERCIAL_CLEAR);

		case INDUSTRIAL:
			return applyZone(eff, INDUSTRIAL_CLEAR);
	...
	private void fixSingle(ToolEffectIfc eff)
	{
		...
		if (isRoadDynamic(tile))
		{
			...
			eff.setTile(0, 0, ROAD_TABLE[adjTile]);
		...
		else if (isRailDynamic(tile))
		{
			...
			eff.setTile(0, 0, RAIL_TABLE[adjTile]);
		...
		else if (isWireDynamic(tile))
		{
			...
			eff.setTile(0, 0, WIRE_TABLE[adjTile]);
		} //end if on a rail tile
