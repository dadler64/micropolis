...
class MapScanner extends TileBehavior
{
	...
	{
		boolean powerOn = checkZonePower();
		if (!city.noDisasters && PRNG.nextInt(city.MELTDOWN_TABLE[city.gameLevel]+1) == 0) {
	...
	void doFireStation()
	{
		...
		city.fireStationMap[ypos/8][xpos/8] += z;
	}
	...
	void doPoliceStation()
	{
		...
		city.policeStationMap[ypos/8][xpos/8] += z;
	}
	...
	void doStadiumEmpty()
	{
		...
		if (powerOn)
		{
			if (((city.cityTime + xpos + ypos) % 32) == 0) {
				drawStadium(FULL_STADIUM);
				city.setTile(xpos+1,ypos, (char)(FOOTBALL_GAME_1));
				city.setTile(xpos+1,ypos+1,(char)(FOOTBALL_GAME_2));
			}
		}
	}
...
//FIXME- should be 'else if'
	...
	void doResidential()
	{
		...
		if (tile == RESIDENTIAL_CLEAR || PRNG.nextInt(8) == 0)
		{
	...
	private void doResidentialIn(int pop, int value)
	{
		...
		int z = city.pollutionMap[ypos/2][xpos/2];
		if (z > 128)
			return;
	...
	{
		int base = (value * 5 + density) * 9 + COMMERCIAL_ZONE_BASE;
		zonePlop(base);
	}

	void indPlop(int density, int value)
	{
		int base = (value * 4 + density) * 9 + INDUSTRIAL_ZONE_BASE;
		zonePlop(base);
	}

	void residentialPlop(int density, int value)
	{
		int base = (value * 4 + density) * 9 + RESIDENTIAL_ZONE_BASE;
		zonePlop(base);
	}
	...
	private void doResidentialOut(int pop, int value)
	{
		...
		if (pop == 16)
		{
			...
			city.setTile(xpos, ypos, RESIDENTIAL_CLEAR);
		...
		if (pop < 16)
		{
			...
			for (int x = xpos-1; x <= xpos+1; x++)
			{
				for (int y = ypos-1; y <= ypos+1; y++)
				{
					if (city.testBounds(x,y))
					{
						int loc = city.map[y][x] & LOW_MASK;
						if (loc >= LHTHR && loc <= HHTHR)
						{ //little house
							city.setTile(x, y, (char)(Brdr[z] + RESIDENTIAL_CLEAR - 4));
							return;
						}
					}
					z++;
	...
	int evalResidential(int traf)
	{
		...
		value -= city.pollutionMap[ypos/2][xpos/2];
	...
	int getCRValue()
	{
		...
		lval -= city.pollutionMap[ypos/2][xpos/2];
	...
	{
		city.rateOfGrowthMem[ypos/8][xpos/8] += 4*amount;
	}
