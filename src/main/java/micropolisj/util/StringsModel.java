...
class StringsModel extends AbstractTableModel
{
		...
		throws IOException
	{
		...
		System.out.println(StringsModel.class.getResource("/").toString());
		p.load(StringsModel.class.getResourceAsStream("/main/java/micropolisj/" +file+".properties"));
	...
	{
		File d = new File(workingDirectory, "main/java/micropolisj");
		return new File(d,
			file
		...
		throws IOException
	{
		...
		for (String file : FILES)
		{
			Properties p = new Properties();
			{
				...
				String s = "/main/java/micropolisj/" +file+(localeCode != null ? "_"+localeCode : "") + ".properties";
