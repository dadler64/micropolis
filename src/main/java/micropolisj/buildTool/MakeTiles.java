...
public class MakeTiles {
    ...
    /**
     * Generate the tiles image from an inputted file recipe
     * @param recipeFile File containing the detail fo the tiles image to be generated
     * @param outputDir
     * @throws IOException
     */
    static void generateFromRecipe(File recipeFile, File outputDir) throws IOException {
        Properties recipe = new Properties();
        recipe.load(new InputStreamReader(new FileInputStream(recipeFile), UTF8));

        // count number of images
        String[] tileNames = generateTileNames(recipe);
        int numTiles = COUNT_TILES == -1 ? tileNames.length : COUNT_TILES;

        // actually assemble the image
        BufferedImage buf = new BufferedImage(TILE_SIZE, TILE_SIZE * numTiles, BufferedImage.TYPE_INT_RGB);
